...
export const Route = createFileRoute("/bookmarks")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Bookmarked chapters, formulas, examples and personal notes.",
      },
    ],
  }),
    ...
    <AppShell back={{ to: "/" }} title="My Library">
      ...
      <section className="mb-6 rounded-3xl border border-border bg-card/70 p-5 shadow-[var(--shadow-elegant)] backdrop-blur">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              {progress.completed.length} / {CHAPTERS.length} chapters complete
            </div>
          </div>
        ...
        <div className="mt-3 text-[11px] text-muted-foreground">
          Today: {progress.todayMinutes} min
        </div>
      </section>

      {/* Bookmarks */}
      <h2 className="mb-3 text-sm font-bold uppercase tracking-[0.18em] text-foreground">
        Bookmarks
      </h2>
      {bookmarks.length === 0 && (
        <p className="rounded-2xl border border-border bg-card/60 p-5 text-sm text-muted-foreground">
          You haven't bookmarked anything yet. Tap the <BookmarkIcon size={12} className="inline" />{" "}
          icon inside any chapter.
        </p>
      )}
              ...
              <div key={g.k}>
                ...
                <div className="space-y-2">
                  {g.items.map((b) => (
                    ...
                    >
                      <Link to={b.href as never} className="min-w-0 flex-1">
                        <div className="truncate text-sm font-semibold text-foreground">
                          {formatMathDisplayText(b.title)}
                        </div>
                        {b.subtitle && (
                          <div className="truncate text-[11px] text-muted-foreground">
                            {formatMathDisplayText(b.subtitle)}
                          </div>
                        )}
                      </Link>
                      <button
      ...
      {/* Notes */}
      <h2 className="mb-3 mt-8 text-sm font-bold uppercase tracking-[0.18em] text-foreground">
        My Notes
      </h2>
      {notes.length === 0 ? (
        <p className="rounded-2xl border border-border bg-card/60 p-5 text-sm text-muted-foreground">
