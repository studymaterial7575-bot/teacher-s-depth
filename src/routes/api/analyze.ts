...
function toInlineData(dataUrl: string, fallbackMime: string): GeminiInlineData | null {
  ...
  if (commaIndex >= 0) {
    return { mimeType: fallbackMime, data: dataUrl.slice(commaIndex + 1) };
  }
  return null;
}
  ...
  "videos": [ { "title": string, "query": string } ]
        ...
        for (const f of files) {
          if (f.mime.startsWith("image/") || f.mime === "application/pdf") {
            const inlineData = toInlineData(f.dataUrl, f.mime);
            if (inlineData) {
        ...
        return Response.json(parseJsonContent(content));
      },
    },
  },
});
